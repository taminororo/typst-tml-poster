...
#let sec-background = [
  ...
  #v(0.3cm)
  #block(text(weight: "bold", fill: tml-blue, size: 19pt)[データとグラフ構築：先行研究［若月］の手法で Slack ログを GraphVAE 潜在空間へ落とす])
  #v(0.1cm)

  - 本研究は、Slack のコミュニケーションログをグラフに集計し GraphVAE で潜在空間へ埋め込む先行研究［若月耕紀「Slack チャットログにおける組織内コミュニケーション分析」, TML 研］の手法を土台とする。ノードは各メンバー（学年を付与）、エッジは 2 者間の通信量。
  - 生ログから通信量への集計は、メッセージ種別ごとに「誰への送信か」を定める次の規則による。
  ...
  #table(
    ...
    table.header(
      text(fill: white, weight: "bold", size: 17pt)[メッセージ種別],
      text(fill: white, weight: "bold", size: 17pt)[通信先（カウント先）],
    ),
    [メンションあり], [メンションされた人物],
    [メンションなし], [チャンネルの全メンバー（ブロードキャスト）],
    [スレッド],     [スレッドの全メンバー],
  )

  #v(0.2cm)
  - GraphVAE の入力は分子グラフの枠組みを組織へ転用したもので、接続行列 $A$・ノードペア間の会話量を離散化したエッジ種別 $E$・各ノードの学年 $F$（B1〜M2・卒業生）からなる。
  - 対象は技大祭実行委員会情報局の実 Slack（チャンネル名に "develop"／"team" を含む）。各年 10 月〜翌年 9 月を 1 組織として分割し、実組織 28 件を得る。比較用に、構造の異なる理想型 5 件を基本グラフに加える。
  - メンションなし投稿が全メンバーへ加算されるため、活動メンバー間はほぼ必ずエッジが立ち、実組織はほぼ完全グラフになりやすい。
  - 各基本グラフから一部エッジの通信量を一定量ずつ増減させた拡張グラフを学習データに加える（増減操作と量 $N$ は 2 節）。
]
